...
- [ ] P1 restant (audité, pas encore fait) : FAQ 100% statique (aucun appel à `GET /api/faqs`, seulement nom/date templés dans du texte en dur), aucun affichage public des partenaires/exposants réels (`GET /api/partners`/`/api/exhibitors` jamais appelés — teaser page d'accueil 100% statique), aucune page publique de candidature exposant (`POST /api/exhibitors/apply` existe côté back, jamais consommé côté front)
