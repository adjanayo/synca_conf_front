    ...
    <FormShell>
      <form onSubmit={submit} noValidate>
        <FormSection title="Identité">
          ...
          <Field
            label="Photo professionnelle HD"
            required
            error={errors.photo}
            hint="JPG/PNG, min 1 MB, fond neutre"
            full
          >
            <input
              type="file"
        ...
        <FormSection title="Votre intervention">
          ...
          <Field label="Thématique principale" required error={errors.thematique}>
            <select
              className={inputCls}
              value={f.thematique}
              onChange={(e) => set("thematique", e.target.value)}
            >
              <option value="">— Sélectionner —</option>
              {SPEAKER_THEMES.map((x) => (
                <option key={x}>{x}</option>
              ))}
            </select>
          </Field>
          <Field
            label="Titre provisoire"
            required
            error={errors.titreIntervention}
            hint={`${f.titreIntervention.length}/100`}
            full
          >
            <input
              className={inputCls}
              maxLength={100}
              value={f.titreIntervention}
              onChange={(e) => set("titreIntervention", e.target.value)}
            />
          </Field>
          <Field
            ...
            required
            error={errors.resume}
            hint={`${countWords(f.resume)}/200 mots`}
            full
          >
            <textarea
        ...
        <FormSection title="Logistique & motivation">
              ...
              {["Oui", "Non"].map((v) => (
                <label key={v} className="flex items-center gap-2 text-sm">
                  <input
                    type="radio"
                    name="heb"
                    checked={f.hebergement === v}
                    onChange={() => set("hebergement", v)}
          ...
          <Field
            label="Pourquoi intervenir à Synca Conf ?"
            required
          ...
          <Field label="Accord diffusion vidéo" required error={errors.diffusion} full>
            <select
              className={inputCls}
              value={f.diffusion}
              onChange={(e) => set("diffusion", e.target.value)}
            >
              <option value="">— Sélectionner —</option>
              {SPEAKER_DIFFUSION.map((x) => (
                <option key={x}>{x}</option>
              ))}
            </select>
          </Field>
